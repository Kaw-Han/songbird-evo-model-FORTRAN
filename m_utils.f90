!*******************************************************************************
! SVN $Id: m_utils.f90 14061 2023-04-21 10:00:53Z naq011 $
!*******************************************************************************
! BASE_UTILS:
...
!     compute splines and interpolate
      ...
      vc(2) = 0.0
      skip = .false.
      call pchsp_r8(ic, vc, nloc, x, y, d, incfd, wk, nwk, ierr)
      call pchfe_r8(nloc, x, y, d, incfd, skip, nnloc, xx, yy, ierr)
...
!
      ...
      REAL(kind=DP), INTENT(IN)    :: VC(2), X(N), F(INCFD,N)
      REAL(kind=DP), INTENT(INOUT) :: WK(2,N)
      REAL(kind=DP), INTENT(OUT)   :: D(INCFD,N)
!
!  DECLARE LOCAL VARIABLES.
!
      INTEGER :: IBEG, IEND, INDEX, J, NM1
...
!
      ...
      REAL(kind=DP), INTENT(IN)  :: X(N), F(INCFD,N), D(INCFD,N), XE(NE)
      REAL(kind=DP), INTENT(OUT) :: FE(NE)
...
!
!       ----------------------------------------------------------------
        CALL CHFEV_R8 (X(IR-1),X(IR), F(1,IR-1),F(1,IR), D(1,IR-1),D(1,IR),&
     &                 NJ, XE(JFIRST), FE(JFIRST), NEXT, IERC)
!       ----------------------------------------------------------------
         IF (IERC .LT. 0)  GO TO 5005
!
...
!
      ...
      REAL(kind=DP), INTENT(IN)  :: X1, X2, F1, F2, D1, D2, XE(NE)
      REAL(kind=DP), INTENT(OUT) :: FE(NE)
!
!  DECLARE LOCAL VARIABLES.
!
